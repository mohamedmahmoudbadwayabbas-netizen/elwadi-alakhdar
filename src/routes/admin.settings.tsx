...
function SettingsPage() {
  ...
  // Load authoritative store settings from Supabase.
  useEffect(() => {
    let mounted = true;
    void supabase.from("store_settings").select("*").limit(1).maybeSingle().then(({ data, error }) => {
      if (!mounted) return;
      if (error) { toast.error(`تعذر تحميل إعدادات المتجر: ${error.message}`); setS(DEFAULT_SETTINGS); }
      else setS({ ...DEFAULT_SETTINGS, ...(data ?? {}) } as Settings);
      setLoading(false);
  ...
  // Keep the preview responsive while the explicit Save action remains the persistence boundary.
  useEffect(() => {
    if (isInitialLoadRef.current || !s) return;
    setAutoSaveStatus("saved");
    window.dispatchEvent(new CustomEvent("store_settings_updated", { detail: s }));
    const timer = setTimeout(() => setAutoSaveStatus("idle"), 500);
  ...
  const handleSaveAll = async () => {
    ...
    if (dbError) {
      ...
      toast.error(`تعذر حفظ إعدادات المتجر: ${dbError.message}`);
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 lg:order-1 space-y-5">
          ...
          <Section icon={Truck} title="إعدادات الشحن والكوبونات والطلبات">
            <div className="space-y-4">
              <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-3.5 text-xs leading-relaxed">
                <span className="font-black">خصم أول طلب:</span> لا يتم تفعيله من الواجهة حالياً؛ لأن قاعدة البيانات لا تحتوي على first_order_only، وأي سلوك لأول طلب يجب أن يُنفّذ لاحقاً داخل RPC/server-side وليس عبر منطق العميل.
              </div>
