...
function ZonesPage() {
  ...
  const save = async () => {
    ...
    if (!country || !governorate) {
      toast.error("البلد والمحافظة مطلوبان"); return;
    }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((z) =>
      [z.country, z.governorate, z.city, z.area].filter(Boolean).some((v) => (v as string).includes(s))
    );
  }, [rows, q]);
    ...
    <div className="space-y-5" dir="rtl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">أضف البلد، المحافظة، المدينة والمنطقة مع سعر التوصيل لكل منطقة.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={openCreate} className="rounded-xl hero-gradient text-primary-foreground">
          ...
          <DialogContent dir="rtl" className="max-w-lg">
            ...
            <div className="grid gap-3 py-2">
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="المدينة">
                  <Input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="مدينة نصر" />
                </Field>
                <Field label="المنطقة / الحي">
                  <Input value={form.area} onChange={(e) => setForm({ ...form, area: e.target.value })} placeholder="المنطقة العاشرة" />
                </Field>
                <Field label="رسوم التوصيل (ج.م) *">
                  <Input type="number" min={0} step="0.5" value={form.fee}
                    onChange={(e) => setForm({ ...form, fee: Number(e.target.value) })} />
                </Field>
                <Field label="وقت التوصيل التقريبي (دقيقة)">
                  <Input type="number" min={0} value={form.estimated_minutes ?? ""}
                    onChange={(e) => setForm({ ...form, estimated_minutes: e.target.value ? Number(e.target.value) : null })} />
                </Field>
      ...
      <div className="relative max-w-md">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث بالمحافظة أو المدينة..." className="pe-10" />
      </div>
              ...
              <tbody>
                {filtered.map((z) => (
                  <tr key={z.id} className="border-t border-border">
                    ...
                    <td className="p-3 text-muted-foreground">{z.min_order_amount ? `${z.min_order_amount} ج.م` : "—"}</td>
                    <td className="p-3 text-muted-foreground">{z.estimated_minutes ? `${z.estimated_minutes} د` : "—"}</td>
                    <td className="p-3">
                      <Switch checked={z.is_active} onCheckedChange={() => toggle(z)} />
                    </td>
                    <td className="p-3">
                      <div className="flex items-center gap-1">
                        <Button size="icon" variant="ghost" onClick={() => openEdit(z)} aria-label="تعديل">
                          <Pencil className="h-4 w-4" />
                        </Button>
                        <Button size="icon" variant="ghost" onClick={() => remove(z.id)} aria-label="حذف" className="text-destructive hover:text-destructive">
