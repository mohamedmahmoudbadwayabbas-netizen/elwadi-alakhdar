...
export const Route = createFileRoute("/.lovable/oauth/consent")({
  ssr: false,
  head: () => ({ meta: [{ title: "منح الوصول — الوادي الأخضر" }, { name: "robots", content: "noindex,nofollow" }] }),
    ...
    <main dir="rtl" className="grid min-h-screen place-items-center bg-background px-4">
      <div className="w-full max-w-md rounded-3xl border-2 border-[#C9A86B] bg-card p-7 shadow-elegant">
        ...
        <p className="mt-2 text-center text-sm text-muted-foreground">
          سيتمكن <b>{clientName}</b> من استخدام هذا التطبيق بالنيابة عنك واستدعاء أدواته المفعّلة أثناء تسجيل دخولك.
        </p>
        ...
        {error && <p role="alert" className="mt-3 rounded-lg bg-destructive/10 p-2 text-center text-xs text-destructive">{error}</p>}
