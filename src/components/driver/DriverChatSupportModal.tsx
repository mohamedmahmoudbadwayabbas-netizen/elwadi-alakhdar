...
export function DriverChatSupportModal({
  ...
  const PRESET_MESSAGES = [
    `مرحباً ${customerName}، أنا كابتن التوصيل لطلبك رقم #${orderId.slice(0, 8)} من سمارت ستور. أنا في طريقي إليك الآن وسأصل خلال دقائق بإذن الله 🛵`,
  ...
  const handleOpenSupportWhatsApp = () => {
    const msg = encodeURIComponent(
      `مرحباً فريق الدعم الفني لسمارت ستور، أنا كابتن التوصيل وأحتاج لمساعدة بخصوص الطلب رقم #${orderId.slice(0, 8)} للعميل (${customerName}).`,
    );
